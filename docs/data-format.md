...
Snapshot chunks carry stable chunk id/index plus owning `article_id` and ordered `heading_path`; English body `§N`/`§N-N` headings use the same fields. Vector metadata records full/sample scope, expected/stored coverage, chunk-ID set hashes, model/revision, dimensions, query/document prefixes, and `input_format` (`structured-v1` or `text-v1`). The generation descriptor repeats the adopted input format and validates it against the metadata. The server resolves `current` once and verifies each exact artifact byte stream against `generation.json`.
