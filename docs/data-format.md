...
- `generations/<id>/generation.json`: release descriptor v4 with corpus/index identity, artifact size/digest, and vector input provenance
- `generations/<id>/bm25.krxidx`: required `KRXIDX2\n` gzip-compressed binary snapshot (format v7)
- `generations/<id>/vectors.krxvec`: optional `KRXVEC2\n` vector snapshot
- `generations/<id>/vectors.krxvec.meta.json`: required metadata format v4 companion when vector is present

Snapshot chunks carry stable chunk id/index plus owning `article_id` and ordered `heading_path`; English body `§N`/`§N-N` headings use the same fields. Vector metadata records full/sample scope, expected/stored coverage, chunk-ID set hashes, model/revision, dimensions, query/document prefixes, and `input_format` (`structured-v1` or `text-v1`). The generation descriptor repeats the adopted input format and validates it against the metadata. The server resolves `current` once and verifies each exact artifact byte stream against `generation.json`; individual root-level flat files are legacy compatibility input only.
